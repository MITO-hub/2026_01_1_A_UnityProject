...
public class PlayerMovment : MonoBehaviour
{
    ...
    public float jumpForce = 5.0f;                  //점프 힘 값을 선언한다.
    ...
    public bool isGrounded = true;                    //땅 위에 있는지 체크하는 변수 (true/false)
    ...
    void Update()
    {
        ...
        {
            if (!isGliding)                                                  //글라이딩이 아닐 경우 함수를 통해 활성화
            {
                EnableGlider();
            }
            ...
            if (gliderTimeLeft <= 0)                                        //사용 시간이 다 되면 비활성화
            {
                DisableGlider();
                DisableGlider();
            }
        ...
        //점프 입력
        if (Input.GetButton("Jump") && isGrounded)        //&& 두 값을 만족할 때 -> (스페이스 버튼을 눌렀을 때와 땅 위에 있을 때)
        {
            rb.AddForce(Vector3.up * jumpForce, ForceMode.Impulse);         //위쪽 방향으로 설정한 함수 치 만큼 순간적으로 힘을 가한다.
            isGrounded = false;                                               //점프를 하는 순간 땅에서 떨어졌기 때문에 false로 한다.
            realGround = false;
            coyoteTimeCounter = 0;
        }

        //지면에 닿으면 글라이더 시간 회복 및 글라이더 비활성화
        if (isGrounded)
        {
            if(isGliding)
            {
    ...
    void OnCollisionEnter(Collision collision)                              //유니티에서 지원해주는 충돌 체크 함수
    {
        ...
        {
            realGround = true;                                              //땅과 충돌하면 true로 만들어 준다.
        }
    }

    void OnCollisionStay(Collision collision)                               //지면과 충돌이 유지 되는지 확인
    {
        if (collision.gameObject.tag == "Ground")                           // 충돌이 일어난 물체의 Tag 가 Ground 인 경우
        {
            realGround = true;                                              //땅과 충돌하면 true로 만들어 준다.
    ...
    {
        if (realGround)                                                     //실제 지면에 있으면 코요테 타임 리셋
        {
